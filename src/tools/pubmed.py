...
class PubMedAPIWrapper(BaseModel):
    ...
    def run(self, query: str) -> str:
        ...
        """
        try:
            results = self.load(query[: self.MAX_QUERY_LENGTH])
    ...
    def lazy_load(self, query: str) -> Iterator[dict]:
        ...
        url = (
            ...
            + f"&datetype={self.datetype}"
            + (f"&mindate={self.mindate}" if self.mindate else "")
            + (f"&maxdate={self.maxdate}" if self.maxdate else "")
            + f"&sort={self.sort}"
        )
    ...
    def _parse_article(self, uid: str, text_dict: dict) -> dict:
        ...
        # summary = "\n".join(
        #     [
        #         f"{txt['@Label']}: {txt['#text']}" if "#text" in txt and "@Label" in txt else txt
        #         for txt in ar.get("Abstract", {}).get("AbstractText", [])
        #     ]
        # )
        summary = ar.get("Abstract", {}).get("AbstractText", [])
